...
#batchsize一旦修改  必须修改NUM参数 20241108
batchsize = 0
NUM = 0
updated = True
...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    #保证batchsize内数据属于同一个序列
    def updata_global_var(self,seq_id):
        global NUM
        global updated
        global batchsize
        if seq_id == None:
            updated = True
            return self.seq_id,updated
        if batchsize == NUM:
            self.seq_id = None
            batchsize = 0
            updated = True
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            seq_id,updated = self.updata_global_var(self.seq_id)
            if updated:
                # sample a sequence from the given dataset
                seq_id, visible, seq_info_dict,seq_name = self.sample_seq_from_dataset(dataset, is_video_dataset)
                self.seq_id = seq_id
                updated = False
            else:
                seq_id, visible, seq_info_dict,seq_name = self.sample_seq_from_dataset_fixed(dataset, is_video_dataset,seq_id=seq_id)
    ...
    def sample_seq_from_dataset(self, dataset, is_video_dataset):
        ...
        while not enough_visible_frames:
            ...
            # Sample frames
            seq_info_dict,seq_name = dataset.get_sequence_info(seq_id)
            visible = seq_info_dict['visible']
            enough_visible_frames = visible.type(torch.int64).sum().item() > 2 * (
                    self.num_search_frames + self.num_template_frames) and len(visible) >= 20

            enough_visible_frames = enough_visible_frames or not is_video_dataset
        return seq_id, visible, seq_info_dict,seq_name
